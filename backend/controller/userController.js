...
exports.getFollowers = function (req, res, next){
  ...
  .exec(function (err, user){
    ...
    User.find({ _id: { $in: user.followers }}).exec(function (err1, users){
      const l =[];
      for (let index = 0; index < users.length; index++) {
        l.push({id: users[index]._id, name: users[index].username, photo: users[index].photo}); 
      }
      res.send(l);
    });
  });
...
exports.getFollowing = function (req, res, next){
  ...
  .exec(function (err, user){
    ...
    User.find({ _id: { $in: user.following }}).exec(function (err1, users){
      const l =[];
      for (let index = 0; index < users.length; index++) {
        l.push({id: users[index]._id, name: users[index].username, photo: users[index].photo}); 
      }
